...
export default function ManufacturersList({ data }: IManufacturersList) {
  return (
    <Table>
      <TableHeader>
        <TableRow>
      ...
      <TableBody>
        {
          data.map((item, index) => (
            <TableRow key={item.id}>
              ...
              <TableCell className="grid place-items-center py-2"><button className="grid place-items-center bg-accent w-8 h-8 rounded-full hover:bg-gray-200"><Eye className="text-gray-500 w-5 h-5" /></button></TableCell>
            </TableRow>
          ))
        }
      </TableBody>
